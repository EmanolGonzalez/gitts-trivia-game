...
// Sugerencias
const iconOptions = ['Sparkles','Atom','Building','Trophy','Cpu','Palette','Book','Globe','Mic','Film']
...
onMounted(async () => {
  ...
  if (game.questions.length === 0 || game.categories.length === 0) {
    try { await game.loadFromPublicData() } catch {}
  }
  // Copias locales
...
function validateQuestion(form: Partial<Question>): string[] {
  ...
  // Unicidad de ID
  const dup = questions.value.some(q => q.id === form.id && (editorMode.value === 'create' || q.id !== (form as Question).id))
...
function validateCategory(form: Partial<Category>): string[] {
  ...
  const dup = categories.value.some(c => c.id === form.id && (editorMode.value === 'create' || c.id !== (form as Category).id))
...
function removeCategory(c: Category) {
  ...
  if (used) {
    askConfirm(`La categoría "${c.name}" tiene preguntas asociadas. ¿Eliminar y reasignar esas preguntas a "uncategorized"?`, () => {
      ...
      questions.value = questions.value.map(q => q.categoryId === c.id ? { ...q, categoryId: 'uncategorized' } : q)
...
function importJsonChange(e: Event) {
  ...
  reader.onload = () => {
    try {
      ...
      questions.value  = data.questions.map(q => ({ id: q.id, categoryId: q.categoryId, text: q.text, answer: q.answer, points: q.points }))
    } catch {
...
function fixMissingCategoryRefs() {
  ...
  questions.value = questions.value.map(q => validIds.has(q.categoryId) ? q : { ...q, categoryId: 'uncategorized' })
}
  ...
  <div class="mx-auto max-w-7xl px-4 py-6">
    ...
    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
      ...
      <div class="flex items-center gap-2">
        <button @click="exportJson" class="btn btn-sm bg-slate-800 hover:bg-slate-700 text-white rounded-xl px-4 py-2">
          Exportar JSON
        </button>
        <button @click="importJsonClick" class="btn btn-sm bg-slate-100 hover:bg-slate-200 text-slate-900 rounded-xl px-4 py-2">
          Importar JSON
        </button>
        <input ref="importInput" type="file" accept="application/json" class="hidden" @change="importJsonChange" />
        <button @click="publishToGame" class="btn btn-sm bg-blue-600 hover:bg-blue-500 text-white rounded-xl px-4 py-2">
          Publicar al juego
        </button>
      </div>
    </div>
    ...
    <div class="mt-4 grid grid-cols-2 gap-3 md:grid-cols-4">
      ...
      <div class="rounded-2xl border border-white/10 bg-white/5 p-4">
        <div class="text-xs uppercase opacity-70">Tiempo por pregunta</div>
        <div class="text-2xl font-semibold">{{ game.defaultTimeLimit }}s</div>
      </div>
      <div class="rounded-2xl border border-white/10 bg-white/5 p-4">
    ...
    <div class="mt-6 flex items-center gap-2">
      ...
      <button
        class="rounded-xl px-4 py-2 text-sm"
        :class="viewTab==='categories' ? 'bg-blue-600 text-white' : 'bg-white/5 hover:bg-white/10'"
      ...
      <div class="ml-auto flex items-center gap-2" v-if="viewTab==='questions'">
        ...
        />
        <select v-model="selectedCategory" class="rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm">
          <option value="all">Todas las categorías</option>
          <option v-for="c in categories" :key="c.id" :value="c.id">
            {{ c.name }}
          </option>
        </select>
        <select v-model="pointsFilter" class="rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm">
          <option value="all">Todos los puntajes</option>
          <option :value="100">100 pts</option>
          <option :value="150">150 pts</option>
          <option :value="200">200 pts</option>
        </select>
        <select v-model="sortBy" class="rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm">
          <option value="default">Orden por ID</option>
          <option value="category">Categoría</option>
          <option value="points">Puntos</option>
          <option value="text">Texto</option>
        </select>
        <button @click="openQuestionCreate" class="btn bg-green-600 hover:bg-green-500 text-white rounded-xl px-4 py-2 text-sm">
          Nueva pregunta
        </button>
      </div>

      <div class="ml-auto flex items-center gap-2" v-else>
        <button @click="openCategoryCreate" class="btn bg-green-600 hover:bg-green-500 text-white rounded-xl px-4 py-2 text-sm">
          Nueva categoría
        </button>
        <button @click="normalizeText" class="btn bg-white/5 hover:bg-white/10 rounded-xl px-4 py-2 text-sm">Normalizar textos</button>
        <button @click="fixMissingCategoryRefs" class="btn bg-white/5 hover:bg-white/10 rounded-xl px-4 py-2 text-sm">Arreglar referencias</button>
        <button @click="reindexQuestionIds" class="btn bg-white/5 hover:bg-white/10 rounded-xl px-4 py-2 text-sm">Reindexar IDs</button>
      </div>
    </div>

    <!-- Tabla de preguntas -->
    <div v-if="viewTab==='questions'" class="mt-4 overflow-hidden rounded-2xl border border-white/10">
      <table class="min-w-full border-collapse">
        ...
        <tbody class="divide-y divide-white/10">
          <tr v-for="q in filteredQuestions" :key="q.id" class="hover:bg-white/5">
            ...
            <td class="px-4 py-3">
              <div class="flex items-center gap-2">
                <button class="rounded-lg bg-white/5 px-3 py-1 text-sm hover:bg-white/10" @click="openQuestionEdit(q)">Editar</button>
                <button class="rounded-lg bg-red-600/90 px-3 py-1 text-sm text-white hover:bg-red-600" @click="removeQuestion(q)">Eliminar</button>
              </div>
            </td>
          </tr>
    ...
    <div v-else class="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
      ...
      >
        ...
        <div class="mt-3 flex gap-2">
          <button class="rounded-lg bg-white/5 px-3 py-1 text-sm hover:bg-white/10" @click="openCategoryEdit(c)">Editar</button>
          <button class="rounded-lg bg-red-600/90 px-3 py-1 text-sm text-white hover:bg-red-600" @click="removeCategory(c)">Eliminar</button>
        </div>
      </div>
      <div v-if="categories.length === 0" class="col-span-full rounded-2xl border border-white/10 bg-white/5 p-8 text-center">
    ...
    >
      <div class="w-full md:max-w-2xl rounded-t-2xl md:rounded-2xl bg-slate-900 border border-white/10 shadow-xl">
        <div class="flex items-center justify-between border-b border-white/10 px-4 py-3">
          <div class="text-lg font-semibold">
            {{ editorMode==='create' ? 'Crear' : 'Editar' }} {{ editorType==='question' ? 'pregunta' : 'categoría' }}
          </div>
          <button class="rounded-lg px-3 py-1 text-sm hover:bg-white/10" @click="showEditor=false">Cerrar</button>
        </div>

        <div class="px-4 py-4">
          <!-- Errores -->
          <div v-if="formErrors.length" class="mb-3 rounded-xl border border-red-500/30 bg-red-500/10 p-3 text-sm text-red-200">
          ...
          <div v-if="editorType==='question'" class="grid grid-cols-1 gap-3 md:grid-cols-2">
            <div class="md:col-span-1">
              ...
              <input v-model="qForm.id" type="text" class="w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2 outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            <div class="md:col-span-1">
              <label class="mb-1 block text-sm opacity-80">Puntos</label>
              <input v-model.number="qForm.points" type="number" min="50" step="50" class="w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2 outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            <div class="md:col-span-2">
              <label class="mb-1 block text-sm opacity-80">Texto</label>
              <textarea v-model="qForm.text" rows="2" class="w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2 outline-none focus:ring-2 focus:ring-blue-500"></textarea>
            </div>
            <div class="md:col-span-2">
              <label class="mb-1 block text-sm opacity-80">Respuesta</label>
              <textarea v-model="qForm.answer" rows="2" class="w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2 outline-none focus:ring-2 focus:ring-blue-500"></textarea>
            </div>
            <div class="md:col-span-2">
              <label class="mb-1 block text-sm opacity-80">Categoría</label>
              <select v-model="qForm.categoryId" class="w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2 outline-none focus:ring-2 focus:ring-blue-500">
          ...
          <div v-else class="grid grid-cols-1 gap-3 md:grid-cols-2">
            <div class="md:col-span-1">
              <label class="mb-1 block text-sm opacity-80">ID</label>
              <input v-model="cForm.id" type="text" class="w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2 outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            <div class="md:col-span-1">
              <label class="mb-1 block text-sm opacity-80">Icono</label>
              <select v-model="cForm.icon" class="w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2 outline-none focus:ring-2 focus:ring-blue-500">
                <option v-for="i in iconOptions" :key="i" :value="i">{{ i }}</option>
              </select>
            </div>
            <div class="md:col-span-2">
              <label class="mb-1 block text-sm opacity-80">Nombre</label>
              <input v-model="cForm.name" type="text" class="w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2 outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
          </div>
        </div>

        <div class="flex items-center justify-end gap-2 border-t border-white/10 px-4 py-3">
          <button class="rounded-xl px-4 py-2 text-sm hover:bg-white/10" @click="showEditor=false">Cancelar</button>
          <button class="rounded-xl bg-blue-600 px-4 py-2 text-sm text-white hover:bg-blue-500" @click="saveEditor">
            Guardar
          </button>
        </div>
      </div>
    ...
    >
      <div class="w-full max-w-md rounded-2xl border border-white/10 bg-slate-900 p-4">
        ...
        <div class="mt-4 flex items-center justify-end gap-2">
          <button class="rounded-xl px-4 py-2 text-sm hover:bg-white/10" @click="confirmDialog.open=false">Cancelar</button>
          <button class="rounded-xl bg-red-600 px-4 py-2 text-sm text-white hover:bg-red-500" @click="confirmDialog.onConfirm">Confirmar</button>
        </div>
      </div>
    </div>
  </div>
...
.btn { @apply rounded-xl px-3 py-2 text-sm transition; }
.text-muted-foreground { color: #9aa4b2; }
</style>
